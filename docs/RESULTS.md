...
Generated by `make report` from the artefacts in `results/`. Every figure below
comes from a run on the machine that produced this file; nothing is quoted from
...
| C2 | architecture relies on MQTT | Abstract, III | quita | `adapters/mqtt/fdt_mqtt.c` |
| C3 | integrates Ardupilot, WeBots, other simulations | Abstract | quita | `adapters/mavlink/fdt_mavlink.c` |
...
| C8 | near-real-time 3D reference | I (iii) | quita | `adapters/webots/fdt_webots_controller.c` |
...
| C22 | WeBots CPU: 10 % then under 1 % | V-A | fronteira | `needs the WeBots renderer` |
...
--- per fleet size ---
   vessels   delta worst    delta mean     cpu/DTI     inj/frame   feasible
         1        1.2 us        0.1 us    17.457 %        0.4 us        yes
         2        0.3 us        0.1 us     8.982 %        0.6 us        yes
         4        2.6 us        0.2 us     3.602 %        1.2 us        yes
         8        7.1 us        0.5 us     1.933 %        2.5 us        yes
        16        9.3 us        0.9 us     0.806 %        6.2 us        yes
        25        7.1 us        1.3 us     0.453 %        9.9 us        yes
        32        2.1 us        1.1 us     0.338 %        9.3 us        yes
        48        2.9 us        1.8 us     0.197 %       17.0 us        yes
        64        7.6 us        2.4 us     0.129 %       26.9 us        yes

--- DTI side — what Section V-B calls negligible ---
  cpu per DTI                0.1287 %               paper: < 1 %                              [OK]
  worst delta, 64 vessels    7.6 us                 paper: < 125 ms deadline                  [OK]
  DTI ceiling here           >= 64 vessels          paper: not the paper's 25                 [OK]

--- injector side — what actually capped the paper at ~25 ---
  injector ceiling here      >= 64 vessels          paper: > 25 boats, same computer          [OK]
...
--- frame integrity (Section V-B pathology) ---
  partial frames: 0   double updates: 0
  The loopback transport loses nothing, so the pathology does not
  appear here. It is exercised over a lossy path in tests/test_wire.c.
...
--- measurement 1 — delta compute time (Section IV) ---
  p50     0.13 us   p95     0.18 us   p99     0.21 us   worst     1.37 us
  delta under the deadline   1.37 us worst          paper: delta in less than 125 ms is feasible [OK]

--- measurement 2 — actuation round trip (Section V-A) ---
  I^t published -> A^t delivered
  p50     1.48 us   p95     2.03 us   p99     3.41 us   worst    32.32 us
...
--- why they are not one number ---
  delta worst    :     1.37 us  (compute)
  round trip p50 :     1.48 us  (compute + transport)
...
--- frame period held under a stepping fleet ---
  ...
  |deviation| p95  :    0.081 ms
  |deviation| p99  :    0.887 ms
  |deviation| worst:    0.929 ms
  pacer overruns   :        0
